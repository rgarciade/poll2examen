...
export default class ConfirmDialog extends Component {
  ...
  render() {
    ...
    const state = action && action.state ? action.state : '';
    return (
      <Modal isOpen={isOpen} onRequestHide={() => this.handleCancelClick()} backdrop keyboard>
        ...
        <div className="modal-body">
          <p>{(state !== "unlocked") ? msg : "can not remove"}</p>
        </div>
        <div className="modal-footer">
          <button className="btn btn-default" ref="cancelButton" onClick={() => this.handleCancelClick()}>
            Cancel
          </button>
          {(state !== "unlocked") ? <button className="btn btn-danger" onClick={() => this.handleConfirmClick()}>Do it!
                                    </button> : false
          }
        </div>
      </Modal>
