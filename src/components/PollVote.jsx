...
export default class PollVote extends Component {
  ...
  handleVoteClick(Poll, idEntry, auth) {
    (Poll.userslist && Poll.userslist[auth.id]) ? this.props.changeVoteEntry(Poll, idEntry, auth) : this.props.voteEntry(Poll, idEntry, auth);
  }
              ...
              <ul className="list-group">
                {
                  Object.keys(entries).map( (id, index) =>
                    <li className="list-group-item" key={index}>
                      { entries[id].title }
                      { poll.state === 'unlocked' && auth.authenticated === true ? <span onClick={ () => this.handleVoteClick(poll, id, auth) } className="action-element glyphicon glyphicon-arrow-up"/> : null }
                      <br/>
...
PollVote.propTypes = {
  ...
  changeVoteEntry:PropTypes.func.isRequired,
